...
int RtmpInit()
{
	...
	do {
			...
			if (RtmpPubInit(gAppContext.pRtmpc) != QN_SUCCESS) {
				...
				QnDemoPrint(DEMO_ERR, "%s[%d] RtmpPubInit  failed.\n", __func__, __LINE__);
				continue;
			}
			int s32Ret = QN_FAIL;
			s32Ret = RtmpPubConnect(gAppContext.pRtmpc);
			if (s32Ret != QN_SUCCESS) {
		//	if (RtmpPubConnect(gAppContext.pRtmpc) != QN_SUCCESS) {
				RtmpPubDel(gAppContext.pRtmpc);
				QnDemoPrint(DEMO_ERR, "%s[%d] RtmpPubConnect failed.s32Ret :%d\n", __func__, __LINE__, s32Ret);
				gAppContext.pRtmpc = NULL;
			}
			else {
				break;
			}
			QnDemoPrint(DEMO_ERR, "%s[%d] RtmpInit loop.\n", __func__, __LINE__);
	} while(1);
